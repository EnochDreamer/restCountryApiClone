...
def get_countries():
    ...
    return jsonify([
        country.format() for country in countries
    ]
    )
@app.route('/continents/<int:continent_id>')
def get_countries_by_continent(continent_id):
    continent=Continents.query.filter(Continents.id==continent_id).one_or_none()
    if continent is None:
        abort(404)
    return jsonify(
[country.format() for country in continent.countries]
)
...
def add_country(paylad):
    ...
    if ('common_name' or 'currency' or 'population' or 'cca3' or 'official_language' or 'continent_id') not in body:
        abort(400)
    new_country=Countries(common_name=toLower(body['common_name']),currency=toLower(body['currency']),population=body['population'],cca3=toLower(body['cca3']), official_language=toLower(body['official_language']),continent_id=int(body['continent_id']))
